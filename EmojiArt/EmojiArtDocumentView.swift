...
//    init(document: EmojiArtDocumentVM) {
//        self.document = document
//        _chosenPalette = State(warppedValue: self.document.defaultPalette)
//    }
    ...
    var body: some View {
            ...
            .navigationBarItems(trailing: Button(action: {
                ...
                Image(systemName: "doc.on.clipboard").imageScale(.large)
                    .alert(isPresented: $explainBackgroundPaste) {
                        Alert(title: Text("error"), message: Text("粘贴失败！"), dismissButton: .default(Text("确认")))
                    }
            }))
            }
            .zIndex(-1)
        ...
        .alert(isPresented: $confirmBackgroundPaste) {
            Alert(title: Text("提示"), message: Text("添加背景图片，链接：\(UIPasteboard.general.url?.absoluteString ?? "空")"), primaryButton: .default(Text("确定")) {
